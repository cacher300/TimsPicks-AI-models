...
CUTOFF_DATE = None                        
MODEL_OUT = "score_lgbm_calibrated.joblib"
FEATURES_OUT = "score_lgbm_features.csv"
RAW_IMPORTANCE_OUT = "lgbm_feature_importance.csv"
...
def train_calibrated_lgbm(X_train, y_train):
    pos = y_train.sum()
    neg = len(y_train) - pos
    pos_w = (neg / max(pos, 1)) if pos > 0 else 1.0
    ...
    cal = CalibratedClassifierCV(base_lgbm, method="isotonic", cv=cv)
...
def main():
    ...
    cal, pos_w = train_calibrated_lgbm(X_train, y_train)
